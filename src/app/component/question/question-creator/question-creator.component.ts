...
export class QuestionCreatorComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  /**
   * True only after {@link runLayout}'s RAF pass assigns {@link paginatedPages} — avoids auto-fit overlay 100%
   * on paths that bail early with another {@link scheduleLayout}.
   */
  private layoutDidCommitPaginatedPages = false;
  /**
  ...
  private buildMcqAnswerKeySetBlocks(multiMcq: boolean) {
    ...
    return buildMcqAnswerKeySetBlocks({
      multiSet: multiMcq,
      mcqSetLetters: QuestionCreatorComponent.MCQ_SET_LETTERS,
      orderedMcqBySet: orderedBySet,
      singleSetMcqs: this.allCanonicalPreviewQuestions.filter((q) => this.questionIsMcqType(q)),
  ...
  private buildAnswersExplanationsExportQuestionList(multiMcq: boolean): any[] {
    ...
    return buildAnswersExplanationsExportQuestions({
      ...
      mcqSetLetters: QuestionCreatorComponent.MCQ_SET_LETTERS,
      orderedMcqBySet: orderedBySet,
      singlePreviewList: this.allCanonicalPreviewQuestions,
      formatOption: (r) => this.getOptionDisplayText(r),
      isCreativeType: (q) => this.questionIsCreativeType(q as { type?: unknown }),
      isMcqType: (q) => this.questionIsMcqType(q as { type?: unknown }),
    ...
    if (this.selectionHasMcqType()) {
      ...
      if (questions.length > 0) {
        ...
        for (const fmt of fmts) {
          ...
          downloadNames.push(fmt === 'pdf' ? `${baseName}-mcq-answers.pdf` : `${baseName}-mcq-answers.docx`);
        }
      }
    }
