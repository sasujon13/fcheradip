...
export class Merit6Component implements OnInit {
  baseUrl: string = 'https://cheradip.com/api/merit6/'
  ...
  getVacancies(page: number): void {
    ...
    this.http.get(`${this.baseUrl}`, { params }).subscribe((data: any) => {
      if (data.count === 0) {
        ...
        this.recordRange = `Displaying <b>${startRecord}-${endRecord}</b> records of <b>${this.totalCount}</b> Total (3996+60,634)=64,630 Records!`;
        setTimeout(() => {
