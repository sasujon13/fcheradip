...
export class Recommend7Component implements OnInit {
  baseUrl: string = 'https://cheradip.com/api/recommend/'
  ...
  getVacancies(page: number): void {
    ...
    this.http.get(`${this.baseUrl}`, { params }).subscribe((data: any) => {
      if (data.count === 0) {
        ...
        this.recordRange = `Displaying <b>${startRecord}-${endRecord}</b> records of <b>${this.totalCount}</b> of total xxxxxxx Records!`;
