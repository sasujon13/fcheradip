...
export class Recommend6Component implements OnInit {
  baseUrl: string = 'https://cheradip.com/api/recommend6/'
  baseUrl2: string = 'https://cheradip.com/api/institute/'
  @ViewChild('scrollContainer', { static: true }) scrollContainer!: ElementRef;

  districts2: string[] = [];
  selectedDistricts2: string[] = [];
  ...
  getVacancies(page: number): void {
    ...
    this.http.get(`${this.baseUrl}`, { params }).subscribe((data: any) => {
      if (data.count === 0) {
        ...
        this.recordRange = `Displaying <b>${startRecord}-${endRecord}</b> records of <b>${this.totalCount}</b> of total 41,627 Records!`;
